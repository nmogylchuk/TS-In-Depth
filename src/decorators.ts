...
export function writable(isWritable: boolean) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor): PropertyDescriptor {
        console.log(`writable decorator is called with value: ${writable}`);
...
export function timeout(ms: number = 0) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor): PropertyDescriptor {
        const originMathod = descriptor.value;
        // descriptor.value = function (...args: Parameters<typeof originMathod>[]) {};
        descriptor.value = function (...args: any[]) {
            if (window.confirm('Are you sure?')) {
                setTimeout(() => {
                    originMathod.apply(this, args);
                }, ms);
            }
        };
        return descriptor;
...
export function format(pref: string = 'Ms./Mrs.') {
